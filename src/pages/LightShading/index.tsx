...
function CanvasContent() {
  ...
  useFrame(({ clock }) => {
    if (!torusRef.current || !suzanneRef.current || !sphereRef.current) return;
  ...
  return (
    <group>
      ...
      <mesh position={pointLight1Position} visible={pointLight1Enabled}>
        <icosahedronGeometry args={[0.1, 2]} />
        <meshBasicMaterial color={pointLight1Color} />
      </mesh>
      <mesh position={pointLight2Position} visible={pointLight2Enabled}>
        <icosahedronGeometry args={[0.1, 2]} />
...
export default function LightShading() {
  return (
    <div className="h-svh bg-slate-800 w-full relative">
      <div className="absolute w-[400px] right-0 z-10">
        <Leva fill />
      </div>
      <Canvas>
        <CanvasContent />
